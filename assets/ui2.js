...
function totalLatentBenefit(companies) {
    let totalBenefit = 0;
    for (const c of companies) {
        totalBenefit += c.latentBenefit || 0;
    }
    return totalBenefit;
}

function workersEstimatedDailyProfit(company) {
    let totalWorkersProfit = 0;
    const workers = company.workersInfo || [];
    for (const w of workers) {
        totalWorkersProfit += w.netProfit * w.estimatedDailySessions || 0;
    }
    return totalWorkersProfit;
}

// --- CALCUL BENEFICE TOTAL PAR JOUR ---
function totalestimatedBenefitEconomy(companylist) {
    let totalBenefit = 0;
    const totalLatent = totalLatentBenefit(companylist);
    totalBenefit += totalLatent;
    let totalWorkersProfit = 0;
    for (const c of companylist) {
        const workers = c.workersInfo || [];
        for (const w of workers) {
            totalWorkersProfit += w.netProfit * w.estimatedDailySessions || 0;
        }
    }
    totalBenefit += totalWorkersProfit;

    let totalUserProfit = 0;
    for (const c of companylist) {
        const profit_user= (c.userPPPerSession) * (1 + (c.bonus)) * (c.netValuePerPP);
        totalUserProfit += profit_user * c.usersession || 0;
    }
    const averageUserProfit = totalUserProfit / companylist.length;
    totalBenefit += averageUserProfit;
    return {totalBenefit, totalLatent, totalWorkersProfit, averageUserProfit};
}

function evaluateCompanyStorage(company) {
    const storageCapacity = company.storageCapacity || 0;
    const latentProduction = company.latentProduction || 0;
    const workers = company.workersInfo || [];
    let totalCapacity = 0;
    for (const w of workers) {
        totalCapacity += w.ppStocked * w.estimatedDailySessions || 0;
    }
    totalCapacity += latentProduction;
    // Pourcentage d'utilisation du stockage
    const usagePercent = storageCapacity > 0 ? (totalCapacity / storageCapacity) : 0;
    let riskLevel = '';
    if (usagePercent >= 1) {
        riskLevel = 'Élevé';
    ...
    return { storagePercentage: usagePercent, storageCapacity, currentStorage: totalCapacity, riskLevel };
}
...
function displayWorkerDetails(workers, minBonusRequired, company) {
    ...
    const nbWorkers = company.nb_workers || 0;
    ...
    workers.forEach(worker => {
        ...
        const session = formatNumber(worker.estimatedDailySessions);
...
export function displayCompanies(companiesList) {
    ...
    const totalLatent = totalLatentBenefit(companiesList);
    const totalBenefit = totalestimatedBenefitEconomy(companiesList).totalBenefit;
    const workersProfit = totalestimatedBenefitEconomy(companiesList).totalWorkersProfit;
    const averageUserProfit = totalestimatedBenefitEconomy(companiesList).averageUserProfit;
    companiesTitle.innerText = `Mes Compagnies - Bénéfice Estimé Total par Jour : ${formatCurrency(totalBenefit, 3)}`;

    let html = `<h3>🏭 Mes Compagnies (${companiesList.length}) - Meilleure Rentabilité : ${name_best}</h3>`;
    html += `<p style="font-size:0.95em; color:white;">⚙️ Bénéfice Latent Total Estimé (Moteur Automatisé): ${formatCurrency(totalLatent, 3)} par jour</p>`;
    html += `<p style="font-size:0.95em; color:white;">🧑‍🔧 Bénéfice Total Estimé des Employés: ${formatCurrency(workersProfit, 3)} par jour</p>`;
    html += `<p style="font-size:0.95em; color:white;">🤗 Bénéfice Total Estimé de l'Utilisateur (ME): ${formatCurrency(averageUserProfit, 3)} par jour</p>`;
    ...
    companiesList.forEach(comp => {
        ...
        const totalprofit = computeCompanyProfit(comp);
        ...
        const latentProduction = comp.latentProduction ? parseFloat(comp.latentProduction).toFixed(3) : '0';
        const latentBenefit = comp.latentBenefit ? formatCurrency(comp.latentBenefit, 3) : 'N/A';
        const latentHtml = `<p style="margin-top:8px; font-size:0.9em; color:white;">⚙️ Production Latente (Moteur Automatisé): ${latentProduction} PP - Bénéfice Latent Estimé: ${latentBenefit} par jour</p>`;
        const workersProfitperDay = workersEstimatedDailyProfit(comp);
        const workerProfithtml = `<p style="margin-top:4px; font-size:0.9em; color:#1ED6C7;">🧑‍🔧 Bénéfice Estimé des Employés: ${formatCurrency(workersProfitperDay, 3)} par jour</p>`;

        const stockageEvaluation = evaluateCompanyStorage(comp);
        const riskLevel = stockageEvaluation.riskLevel;
        const storageCapacity = stockageEvaluation.storageCapacity;
        const storagePercent = (stockageEvaluation.storagePercentage * 100).toFixed(1);
        const colorRisk = stockageEvaluation.storagePercentage >= 1 ? '#ff4444' :
                          stockageEvaluation.storagePercentage >= 0.75 ? '#ffbb33' :
                          stockageEvaluation.storagePercentage >= 0.5 ? '#ffff66' : '#99cc00';
        const storageHtml = `<p style="margin-top:4px; font-size:0.9em; color:${colorRisk};">📦Capacité de stockage: ${storageCapacity} - Utilisation du Stockage: ${storagePercent}% - Risque de surproduction en 24h: ${riskLevel}</p>`;
