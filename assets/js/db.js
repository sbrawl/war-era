...
export async function saveTransactions(transactions) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // La Promise se résout quand TOUTES les opérations (put) sont terminées
        tx.oncomplete = () => {
            console.log(`✅ ${transactions.length} transactions sauvegardées en base`);
            resolve();
        };
        
        tx.onerror = () => {
            console.error("❌ Erreur lors de la sauvegarde en IndexedDB:", tx.error);
            reject(new Error(tx.error?.message || 'Erreur lors du write en IndexedDB'));
        };
    });
}
